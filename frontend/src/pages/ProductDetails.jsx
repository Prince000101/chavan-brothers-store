...
function ProductDetails() {
  ...
  const productLd = product
    ? {
        ...
        image: product.image,
        brand: { "@type": "Brand", name: "Chavan Brothers" },
        offers: {
          "@type": "Offer",
          price: currentPrice,
          priceCurrency: "INR",
  ...
  return (
    <div className="min-h-screen pb-16 sm:pb-20 transition-colors duration-500">
      <SEO
        title={product.name}
        description={product.description || `${product.name} from Chavan Brothers - authentic Mumbai masala, stone-ground fresh since 1937.`}
        keywords={`${product.name}, Chavan Brothers, ${product.category || "masala"}, Mumbai spices, authentic Indian spices`}
        url={`https://chavanbrothers.com/products/${product._id}`}
        image={product.image}
        type="product"
        ld={productLd}
      />
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 mt-14 sm:mt-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
            {relatedProducts.map((item) => (
              <Link to={`/products/${item._id}`} key={item._id} className="group block bg-white dark:bg-charcoal-light p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-spice/20 dark:border-spice/30 transition-all hover:-translate-y-1 hover:shadow-xl">
